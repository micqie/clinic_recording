 ...
 function register($json)
{
  ...
  try {
      ...
      // 6. Insert into tbl_users (self-registration: do NOT force password change)
      $hashedPassword = password_hash($data['password'], PASSWORD_DEFAULT);
      $sql = "INSERT INTO tbl_users (name, email, password, role_id, must_change_password)
              VALUES (:name, :email, :password, :role_id, 0)";
...
function registerPatient($json)
{
    ...
    try {
        ...
        // 5. Insert into tbl_users (self-registration: do NOT force password change)
        $hashedPassword = password_hash($data['password'], PASSWORD_DEFAULT);
        $sql = "INSERT INTO tbl_users (name, email, password, role_id, must_change_password)
                VALUES (:name, :email, :password, :role_id, 0)";
        $stmt = $conn->prepare($sql);
